  ...
  const handleRequestUnban = async () => {
    ...
    try {
      ...
      setToastMessage({
        ...
        desc: response.data.message || 'Unban request submitted successfully.',
        type: 'success'
      });
      setShowToast(true);
    } catch (error) {
      ...
      setToastMessage({
        title: 'Error',
        desc: error.response?.data?.message || 'Failed to submit unban request.',
        type: 'error'
      });
      setShowToast(true);
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setToastMessage({
        ...
        desc: response.data.message || 'Successfully signed in to Client Portal.',
        type: 'success'
      });
      setShowToast(true);
      form.reset();
      ...
      if (error.response?.status === 403 && data?.banned) {
        setBanModal({
          show: true,
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      {banModal.show && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fadeIn">
          <div className="bg-slate-900 border border-red-500/20 rounded-3xl p-8 max-w-md w-full text-center shadow-2xl relative text-white">
            <button 
