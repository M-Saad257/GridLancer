      ...
      <div className="min-h-screen bg-slate-950 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden text-white font-sans">
        ...
        {toast.show && (
          <div className="fixed bottom-8 right-8 z-50 transform transition-all duration-500 ease-out">
            <div className={`bg-slate-900 border ${toast.type === 'success' ? 'border-emerald-500/50 shadow-emerald-500/20' : 'border-rose-500/50 shadow-rose-500/20'} shadow-2xl rounded-2xl p-5 pr-12 flex items-start gap-4 relative`}>
              <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${toast.type === 'success' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}>
                {toast.type === 'success' ? (
              ...
              <div>
                <h4 className="text-white font-bold text-lg">{toast.type === 'success' ? 'Success' : 'Sign In Failed'}</h4>
      ...
      {toast.show && (
        <div className="fixed bottom-8 right-8 z-50 transform transition-all duration-500 ease-out">
          <div className={`bg-slate-900 border ${toast.type === 'success' ? 'border-emerald-500/50 shadow-emerald-500/20' : 'border-rose-500/50 shadow-rose-500/20'} shadow-2xl rounded-2xl p-5 pr-12 flex items-start gap-4 relative`}>
            ...
            <div>
              <h4 className="text-white font-bold text-lg">{toast.type === 'success' ? 'Notification' : 'Error'}</h4>
              <p className="text-slate-400 text-sm mt-1">{toast.message}</p>
            </div>
            <button onClick={() => setToast({ ...toast, show: false })} className="absolute top-4 right-4 text-slate-500 hover:text-white transition-colors cursor-pointer">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
